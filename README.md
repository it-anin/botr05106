...
`change_region` (`left` / `top` / `right` / `bottom` ในพิกัด client) สำคัญมาก
เพราะถ้าคลิกพลาดไปโดนตัวแถว PowerBuilder จะ**เลือกแถวนั้น** ทำให้ pixel เปลี่ยน
เหมือนกัน การตรวจเฉพาะพื้นที่ใต้แถวจึงแยกสองกรณีนี้ออกจากกันได้

**ต้องเว้นอย่างน้อย 1 แถวเต็ม (27 px) จากจุดที่คลิก** — วัดจริงแล้วแถบไฮไลต์ของ
แถวที่ถูกเลือกลากลงไปถึง `y = จุดคลิก + 16` ถ้าตั้ง `top` ชิดกว่านี้จะได้ผลบวกลวง

ข้อจำกัดที่เหลืออยู่: ถ้ามีแถวอื่น**ที่อยู่ใต้ขอบเขตที่ตรวจ**ถูกเลือกค้างไว้ก่อนคลิก
การที่แถวนั้นเสียไฮไลต์ก็นับเป็นการเปลี่ยนเหมือนกัน — ไม่กระทบ flow ที่เริ่มจาก
`login.yaml` เพราะหลังล็อกอินใหม่ยังไม่มีแถวไหนถูกเลือก
