...
ดับเบิลคลิก `promaxx-bot.exe` แล้วมันจะรัน flow ส่งออกให้ทันที **เสร็จแล้วปิดหน้าต่างเอง**
(ไม่ต้องมีไฟล์ `.bat` ช่วย และไม่ต้องกดปุ่มปิด)

หน้าต่างปิดไปแล้วดูผลย้อนหลังได้ที่

| ไฟล์ | ดูอะไร |
|---|---|
| `logs\last_run.json` | สรุปผลรอบล่าสุด — สำเร็จไหม ได้ไฟล์อะไร ใช้เวลาเท่าไร |
| `logs\bot.log` | log เต็มทุกขั้นตอน |
| `screenshots\` | ภาพหน้าจอทุกขั้นตอน (รวมภาพตอนพัง) |
...
**สั่งจาก terminal ยังทำงานเหมือนเดิมทุกอย่าง** — โปรแกรมแยกสองกรณีด้วย
`GetConsoleProcessList` ถ้ามี process เกาะ console อยู่ตัวเดียวแปลว่า Explorer
เพิ่งสร้าง console ใหม่ให้ (= ดับเบิลคลิก จึงเติม flow ให้เอง) ถ้ามีตั้งแต่ 2 ตัวขึ้นไป
แปลว่ามี shell เปิดอยู่ก่อนแล้ว (= สั่งเอง จึงเคารพ argument ที่พิมพ์มา)
...
## สถานะการติดตั้งจริง (2569-09-06)

รันบนเครื่อง **AninMainPC** (Server) — ตั้ง Scheduled Task แล้ว ทดสอบผ่าน

| จุดที่เคยกังวล | ผลจริง |
|---|---|
| จอ **1600×900** ต่างจากเครื่องที่เขียน flow (หน้าต่าง ProMaxx 1936×1048) | ✅ **พิกัดคลิกใน `r05_1.yaml` / `r05_106.yaml` ยังใช้ได้** ไม่ต้องแก้ — เพราะเป็นการคลิกแถวในรายการ ตำแหน่งขึ้นกับความสูงแถว (ฟอนต์/DPI) ไม่ใช่ขนาดหน้าต่าง |
| Node บนเครื่อง Server | ✅ มีอยู่แล้ว v24.18.0 |
| `${USERPROFILE}` ใน flow | ✅ แปลงเป็น `C:\Users\AninMainPC\...` เอง |
| `.env` | ✅ ติดไปกับโฟลเดอร์ `dist\promaxx-bot\` ตอนก๊อป (build_exe ก๊อปให้) |

⚠️ **ห้ามตั้ง Task ตัวนี้ซ้ำบนเครื่องอื่น** — 2 เครื่องรันพร้อมกันจะแย่งกันเขียนตาราง `products`
